...
export default function AdminTags() {
  ...
  const reload = () => apiGet<Tag[]>("/api/tags").then(setList);
  ...
  const create = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await apiPost("/api/admin/tags", form);
      setForm({ name: "", slug: "" });
      reload();
    } catch (e) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <h1 className="text-d-h3 text-text-primary">تگ‌ها</h1>

      <AdminCard>
        <form onSubmit={create} className="flex flex-col md:flex-row gap-3 items-end">
          <label className="flex-1 flex flex-col gap-1.5">
            <span className="text-d-body-sm text-text-secondary">نام تگ</span>
            <input
              required
              value={form.name}
